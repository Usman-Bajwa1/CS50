...
#############################################################################################################################
def indorr():
    say = input("Type any sentence to convert it into lowercase:\n")
    print(f"\n\nHere is your result:\n{say.lower()}")
...
for input and then outputs that same input, 
...
tend to convert emoticons to emoji automatically! In a file called faces.py,
...
and any :( converted to 🙁 (otherwise known as a slightly frowning face). All other 
text should be returned unchanged.
Then, in that same file, implement a function called main that prompts the user for
input, calls convert on that input, and prints the result. You’re welcome, 
...
def Making_faces():
    faces = input("Convert your emojis:\n")
    print(convert(faces))
...
c represents the speed of light (measured approximately as 300000000 meters per
second), per Albert Einstein et al. Essentially, the formula means that mass and 
...
def Einstein():
    c = 300000000
    m = int(input("Enter an integer value for mass: "))
    E = m * (c ** 2)
    print(f"The value of Energy in Joules is: {E}")
